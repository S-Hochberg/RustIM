...
// impl Operation<User, UserInput> for GetUserOperation{
	
// 	async fn execute(&mut self) -> Result<ImResponse<User>, OpError> {
// 		let user = User::new(self.state.clone());
// 		let id = user.id.clone();
// 		let repo = UsersRepo::new(DBDrivers::Postgres);
// 		repo.create(user).await?;
// 		Ok(ImResponse{status: StatusCode::CREATED, body: })
// 	}	
...
// #[cfg(test)]
// mod tests{
//     use crate::{models::user::user::UserInput, operations::{operation::{Operation, OperationsExecutor}, users::create_user_operation::GetUserOperation}, repo::{users_repo::users_repo::UsersRepo, DBDrivers}, test_setups::test_setup};
// 	use axum::http::StatusCode;
// 	use chrono::Utc;
...
// 		let op = GetUserOperation::new(sample_user_input());
...
// 		let op = GetUserOperation::new(user_input.clone());
// 		let first_res = OperationsExecutor::execute_op(op).await;
// 		assert!(first_res.is_ok());
// 		let second_op = GetUserOperation::new(user_input.clone());
